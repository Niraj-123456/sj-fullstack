...
import { resetPassword } from "../https/registerServices";
...
function ResetPassword() {
  ...
  const handleSubmit = async (password, resetForm) => {
    try {
      ...
      const { data } = await resetPassword(phoneNumber, password);
      if (data.success) {
  ...
  return (
    phoneNumber && (
      <EnterPassword heading="Reset Password" handleSubmit={handleSubmit} />
    )
  );
}
